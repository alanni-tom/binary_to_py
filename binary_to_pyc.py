...
class PyInstArchive:
    ...
    def getCArchiveInfo(self):
        ...
        logger.info('[+] Pyinstaller version: {0}'.format(self.pymaj))
    ...
    def parseTOC(self):
        ...
        while parsedLen < self.tableOfContentsSize:
            ...
            try:
                ...
                logger.error(
                    '[!] Warning: File name {0} contains invalid bytes. Using random name {1}'.format(name, newName))
                name = newName
            ...
            parsedLen += entrySize
        logger.warning('[+] Found {0} files in CArchive'.format(len(self.tocList)))
    ...
    def _extractPyz(self, name):
        ...
        with open(name, 'rb') as f:
            ...
            if self.pymaj != sys.version_info.major or self.pymin != sys.version_info.minor:
                ...
                logger.info('[!] Skipping pyz extraction')
                return
            ...
            try:
                ...
                logger.warning('[!] Unmarshalling FAILED. Cannot extract {0}. Extracting remaining files.'.format(name))
                return
    ...
    if arch.open():
        if arch.checkFile():
            if arch.getCArchiveInfo():
                ...
                arch.extractFiles()
                arch.close()
                logger.info('[+] Successfully extracted pyinstaller archive: {0}'.format(filepath))
                last_dir = os.path.basename(filepath.rstrip("\\/"))

                current_file = os.path.abspath(__file__)
                current_dir = os.path.dirname(current_file)

                pyc_to_py(os.path.join(current_dir, last_dir) + "_extracted", output)
